...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/", "/login**", "/**").permitAll().antMatchers("/home/**", "/security/**",

				"/security/verify/**", "/oauth2/**", "/oauth/authorize", "/product/**", "/callback/", "/webjars/**",
				"/error**", "/assets/**", "/file/**/*.*", "/*.html", "/favicon.ico", "/**/*.html", "/**/*.css",
				"/**/*.js").permitAll().antMatchers("/account/editprofile").authenticated().antMatchers("/admin/**")
				.hasAnyRole("ADMIN").anyRequest().authenticated();

		http.formLogin().loginPage("/security/login").loginProcessingUrl("/security/login")
				.defaultSuccessUrl("/home", false).failureUrl("/security/login/error");
		...
		http.oauth2Login().loginPage("/security/login").defaultSuccessUrl("/home", true)
				.failureUrl("/security/login/error").authorizationEndpoint().baseUri("/oauth2/authorization").and()
